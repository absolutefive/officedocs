...
| `::: cards` ~ `:::` | 라운드 카드 그리드 (아래 참고) |
| `::: bar 단어` ~ `:::` | 가로 강조 바: 큰 단어 + 작은 라벨 + 본문 |
...
- **`::: cards`** — 각 `- ` 항목이 카드 한 장. `라벨 | 큰 텍스트 | 보조 설명`(1~3개)으로 나누고, `{tinted}`를 붙이면 포인트 컬러 강조 카드가 됩니다. 카드 2장이면 비교 카드로 보고 사이에 화살표(→)를 넣습니다. 4장 이상이면 2열 그리드로 배치합니다.
- **`::: cards numbered`** — 라벨 대신 번호 원형 배지를 달고 `큰 텍스트 | 보조 설명`으로 해석합니다.
- **`::: bar 단어`** — 가로로 긴 강조 바. 내용 한 줄을 `라벨 | 본문`으로 나눕니다.
- 카드/바의 포인트 컬러는 템플릿 테마의 강조1 색을 따르므로(강조 카드는 강조색 10% 투명 배경) 어떤 템플릿에서든 어울리게 렌더링됩니다. `::: left`/`::: right` 안에서도 쓸 수 있습니다.
